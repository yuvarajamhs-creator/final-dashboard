...
export default function BestPerformingReel() {
    ...
    // Fetch account-level Instagram insights (period-based: Views, Reach, Content interactions for selected date range)
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        fetchInstagramMediaInsights(reelPage, opts)
            .then((data) => {
        ...
        return () => { cancelled = true; };
    }, [reelPage, activeTab, contentFilters.startDate, contentFilters.endDate]);
    ...
    useEffect(() => {
        ...
        return () => { cancelled = true; };
    }, [reelPage, demographicsTimeframe]);
    ...
    const mediaForTab = (mediaInsights?.media || []).filter((m) => {
        ...
        if (activeTab === 'reels') return pt === 'REELS';
        return true;
    });
